...
export default function BusinessSignup() {
  ...
  const handleVerifyAndSignup = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
        setLoading(false);
        localStorage.setItem('businessName', formData.companyName);
        localStorage.setItem('representativeName', `${formData.firstName} ${formData.lastName}`);
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-[#404145]">
      <Navbar variant="skinny" showHustlerHelper={true} />
      ...
      <main className="w-full flex-1 flex flex-col items-center pt-10 pb-16 px-4">
        <div className="w-full max-w-[480px]">
          ...
          <form onSubmit={codeSent ? handleVerifyAndSignup : handleSendOtp} className="space-y-4">
            {!codeSent && (
              <>
                <div className="flex flex-col gap-1">
                  <label htmlFor="companyName" className="text-[13px] font-bold text-black mb-1.5 ml-1">Company name</label>
                  <input 
                    id="companyName"
                    type="text" 
                    required 
            ...
            <div className="flex flex-col gap-1">
              <label htmlFor="phone" className="text-[13px] font-bold text-black mb-1.5 ml-1">{codeSent ? 'Verification Code (sent to ' + formData.phone + ')' : 'Business phone number'}</label>
              {!codeSent ? (
                <div className="flex gap-2">
                    ...
                    <button
                        ...
                        disabled={!formData.phone || loading || !formData.firstName || !formData.lastName || !formData.companyName}
            ...
            >
                {loading ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : (codeSent ? 'Create Business Account' : 'Continue')}
            </button>

            <div className="text-center text-[14px] pt-4">
                <span className="text-slate-600">Looking to join as a hustler? </span>
                <Link to="/signup/hustler" className="text-[#1dbf73] font-semibold hover:underline">Sign up as Hustler</Link>
            </div>
