...
export default function BusinessSignup() {
  ...
  const handleGoogleSignup = () => {
    setLoading(true);
    openGoogleAuthPopup("business", (session) => {
      localStorage.setItem("businessName", session.businessName || "Aria Partner Co");
      localStorage.setItem("representativeName", session.name);
      localStorage.setItem("userEmail", session.email);
      if (session.picture) {
        localStorage.setItem("userPicture", session.picture);
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-[#404145]">
      <Navbar variant="skinny" showHustlerHelper={true} showLoginLink={true} />
      ...
      <main className="w-full flex-1 flex flex-col items-center pt-24 md:pt-24 pb-16 px-4">
        <div className="w-full max-w-[480px]">
          ...
          >
            ...
            <div className="text-center text-[15px] pt-4 font-medium pb-2">
              <span className="text-[#62646a]">
                Looking to join as a hustler?{" "}
              </span>
              <Link
                to="/signup/hustler"
